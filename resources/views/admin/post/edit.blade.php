...
<div class="box box-primary">
	...
	<form enctype="multipart/form-data" role="form" method="post"
		action=" @if(isset($post->id)) {{ route('post.update',$post->id) }} @else {{ route('post.store') }} @endif ">
		{!! csrf_field() !!}
		@if(isset($post->id))
		...
		<div class="box-body">
			...
			<div class="form-group">
				<label>Quantidade de curtidas do Post</label>
				<input class="form-control" name="description" type="text"
					value="@if(isset($post)){{$post->description}}@endif">
			</div>
			<div class="form-group">
				<label>Quantidade de comentários do Post</label>
				<input class="form-control" name="meta_description" type="text"
					value="@if(isset($post)){{$post->meta_description}}@endif">
			</div>
			<div class="form-group">
				<div class="checkbox">
					<label><input name="published" type="checkbox" value="on" @isset($post) @if($post->published)
						checked='checked' @endif @endisset />Publicar</label>
				</div>
			</div>
			<div class="form-group">
				<label>Conteudo</label>
				<textarea class="form-control editor"
					name="content">@if(isset($post)){{$post->content}}@endif</textarea>
			</div>
			<div class="form-group">
				...
				<img class="form-group" src="{{ URL::to('/images/uploads/blog')}}/{{$post->main_image}}"
					width="400px" />
				@endif
			</div>
			...
			<div class="form-group">
				...
				<div class="checkbox">
					<label><input type="checkbox" name="category[]" value="{{$category->id}}" @isset($post)@if(
			...
			<div class="form-group">
				...
				<input class="form-control" name="meta_title" type="text"
					value="@if(isset($post)){{$post->meta_title}}@endif">
			</div>
			<div class="form-group">
				<label>Meta Keywords</label>
				<input class="form-control" name="meta_keywords" type="text"
					value="@if(isset($post)){{$post->meta_keywords}}@endif">
			</div>
		</div>
